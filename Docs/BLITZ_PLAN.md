...
- [x] Evening: finish Sprint 0 — tighten 5 permissive RLS policies, dedupe legacy policies, ~~Postgres upgrade~~, partner_applications review *(done 6/11 morning, ahead of schedule — `sprint0_rls_tightening.sql` applied + verified; **Postgres upgrade deferred to tonight's low-traffic window** — don't upgrade mid-opener-day while waitlist posts drive traffic)*
