...
> - [x] **Bug — collection-membership (`user_jerseys.collection_id`)** — ✅ fixed 6/24 (`ef1e2bb`). Two instances of the same root cause: (1) `AddJerseyToCollectionModal` — adding a kit to a custom collection always failed; now find-or-creates the user_jersey then links via `collection_jerseys`. (2) `EditCollectionModal` delete — would have *destroyed the user's owned kits* on collection delete had the column existed (it errored instead); now removes only the junction links. Both mirror the canonical `useJerseys`/`CollectionDetail` pattern. Worth a QA-day check on the collections feature.
...
- **"Your Kit DNA" — taste analysis + representative kit** *(founder, 6/25)* — analyze a collection's structured attributes ("70% of your kits have this collar / color / pattern / era") and render a single kit that tells the story of your taste. **Two halves:** (1) analysis = aggregation over kit attributes (tractable); (2) the "generated kit" = a spectrum. **Make-or-break dependency: structured design-language attributes.** Today `public_jerseys` has colors / manufacturer / kit_type / era / sponsor but NOT collar style, pattern (stripes/hoops/sash/solid), sleeve, fit. Data only accrues after the fields exist → **the one early action is defining that taxonomy and folding it into the post-beta metadata expansion** (the mid-beta survey can ask collectors which design dimensions feel most "them" to shape the taxonomy). **Phased build, post-launch:** Phase 1 = shareable "Kit DNA card" (dominant colors, most-common collar/pattern/era/brand, % breakdowns) + "spirit kit" = the real catalog kit nearest your centroid; *no generation*, ships on real data, doubles as a launch/OG viral asset. Phase 2 = parameterized SVG kit template driven by the DNA (deterministic, no IP risk). Phase 3 = AI image generation (aspirational; cost + consistency + IP-of-generated-kits questions). Recommendation: Phase 1 is the real v1; don't anchor on AI generation. **Nothing before beta; taxonomy decision is the only pre-launch step.**
  - **Auto-detecting attributes from photos** *(founder, 6/25)* — instead of manual tags, a hybrid pipeline: (1) **colors** via deterministic k-means palette extraction on the segmented shirt (no AI, accurate, scales); (2) **design attributes** (collar/pattern/sleeve/fit/sponsor) via a **vision-language model** (e.g. Claude vision) returning JSON to the taxonomy schema + per-field confidence; (3) **human-confirm** low-confidence fields in the admin approval flow — which also builds a labeled dataset for free; (4) *later, if scale demands* train a cheap classifier the VLM has bootstrapped. **Runs on-upload (pre-fill → confirm) AND as a one-time backfill over every existing `front_image_url`.** Strategic unlock: backfill makes the *whole catalog* attributable retroactively → kills the data-accrual race that otherwise gates Kit DNA. Implication for the near term: **don't build a heavy manual-tagging UI — the future is auto-detect-then-confirm**, so post-beta capture should be thin confirm/override. Image quality is the accuracy ceiling (admin-curated front image = best input). Cost: cents/image VLM + free colors; backfill is a modest one-off. Still post-launch.
...
- **2026-06-25 (map pins + pending-kit dashboard):** Founder noticed a club kit (Samsunspor) had no map pin. Root cause: the map resolved pins via a hardcoded `cityCoords` table (no Turkish cities) and **ignored `clubs.latitude/longitude`** — which the 52 imported EPL/MLS clubs already have. Fixed: (1) map prefers club coords, falls back to city lookup (`83b9b35`); (2) `ClubResolver`/`fetchClubWikidata` now resolves stadium P625 coords so new clubs pin globally; (3) backfill script + ran it for the 2 coord-less clubs (Dortmund, Samsunspor) → **founder: paste `database/data/backfill_club_coords_20260625.sql`**. Also shipped the **pending-kit dashboard fix** in the same commit (stats read pending kits from their submission instead of blank-team rows — important since the thin catalog makes not-found the default path) + FindYourKit expectation copy. 107 tests green.
- **2026-06-25 (club-cataloging polish + QA prep):** Club resolver now in all three surfaces — approval (v1.1 auto-link), standalone quick action, and the **backfill "Unlinked club kits" tool** (new "Find on Wikidata" button: add an unknown club + link all kits with that team name in one pass). AdminClubs UX fix: the 300+ club list buried the backfill tool — moved the tool above the list and capped the list at 50 (search spans all). Founder asked "anything to add before beta?" → assessment: the one genuinely-beneficial build is the **pending-kit dashboard rendering** (with a ~5-kit catalog the not-found path dominates, so most tester kits are pending → `useCollectionStats` renders them as blank-team rows; fix = read identity from the submission like `kitIdentity`), plus expectation-setting copy in FindYourKit; in-app feedback already exists (`DashboardFeedbackBox` → `dashboard_insight_requests` → admin view). Bulk import consciously deferred. *(Pending-kit dashboard fix: pending build.)*
- **2026-06-25 (Sentry live):** Sentry error monitoring verified working in prod. Two real bugs found + fixed en route: (1) `initSentry()` ran before the client entry's `window.onerror`/`onunhandledrejection` assignments, which overwrote Sentry's global handlers — moved init last so it wraps them; (2) the Sprint 0 CSP `connect-src` didn't whitelist Sentry's ingest host, so the browser silently blocked every event despite the SDK reporting "flushed" — added `https://*.ingest.us.sentry.io` to `vercel.json`. Founder set `VITE_SENTRY_DSN` in Vercel; test event confirmed in the dashboard; temp diagnostic button removed. **Caveat noted:** private-mode/tracking-protection/ad-blockers drop a minority of client errors (inherent to client-side tools; not fixed — tunnel revisitable at launch). **Reminder for future integrations: any new third-party host the browser calls must be added to `connect-src` in `vercel.json`.**
- **2026-06-24 (deploy + smoke fixes):** Merged app deployed to prod (`da16c63` → `c9ced08`). Founder smoke-tested the not-found flow; fixed live: (1) QuickAddKit club-typeahead click did nothing — `value`/id wiring bug; now captures `clubId` + seeds the typeahead query from the search term; (2) season free-text → `SeasonTypeahead`, a type-to-filter picker over canonical options (single year + `2025/26`), normalized on blur; (3) club/international toggle now hidden once a club is picked (inferred from its league), shown only for free-typed suggestions. Clubs import cleaned (P1532 + CONIFA filters + curated non-FIFA blocklist) and the `ON CONFLICT (wikidata_id)` → bare `ON CONFLICT DO NOTHING` fix (a name-dup like a pre-existing Toronto FC was aborting the whole atomic insert); **308 clubs live in prod** (22 EPL / 33 MLS / 251 nations + 5 originals). Confirmed `require_front_image` is intended — photoless kits stay pending in the user's collection, not promoted to catalog. 92 tests green. **New founder requests (6/24), approach decided, build pending — see Beta-prep build queue.**
