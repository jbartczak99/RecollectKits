...
> - [x] **Club resolver (v1)** — ✅ done 6/25 (`0bc29c2`): Admin → Quick actions → "Add a club (Wikidata)" → search → preview canonical record → add to `clubs` (find-or-create) + manual fallback. Live-verified (Dortmund w/ aliases/stadium/founded). Rolls unknown clubs into the listing + typeahead. *(v1.1 follow-up, optional: inline on the submission row with auto `public_jerseys.club_id` link at approval, and a `club_suggestions`-queue processor. v1 already covers the core "roll clubs into the db listing" ask.)*
